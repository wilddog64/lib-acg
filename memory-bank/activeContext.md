...
- **COMPLETE:** Azure portal/TAP validation restored as primary path — lib-acg commit `ab6d928` on `feat/v0.1.4`; `bin/acg-credential-test` now validates Azure portal username/password/TAP before any Azure SP metadata branch so `AADSTS700016` from hidden SP fields no longer short-circuits a usable portal session, and `docs/bugs/2026-06-10-azure-sp-info-is-not-cli-auth.md` records why the SP-looking fields are metadata, not the CLI auth source of truth; validation used `shellcheck -S warning bin/acg-credential-test`, `node --check playwright/lib/sandbox.js`, `node --check playwright/providers/azure.js`, `npm test`, and `git diff --check`; commit message: `fix(credential-test): prefer portal TAP over misleading Azure SP metadata`.
