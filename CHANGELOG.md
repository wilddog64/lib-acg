...
### Fixed
- `playwright/lib/sandbox.js`: scope `addLocatorHandler` trigger to the `h3` heading element — broad text regex matched both heading and paragraph, causing Playwright strict-mode violation when the toast appeared
...
- `playwright/lib/sandbox.js`: remove `panelInStartState` heuristic — global DOM check caused bidirectional regression when both provider panels were visible; added conflict-warning guard to skip heuristic when conflict warning is active
...
- `bin/acg-credential-test`: retry sandbox restart when Azure SP credentials absent after extraction
